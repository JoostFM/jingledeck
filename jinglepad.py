...
def get_key_details(deck, key, state):
    ...
    name = decksets.keys[keyset][key][0]
    #if keys[level][key][1] == '': icon = keys[level][key][1]
    icon = "{}.png".format(decksets.keys[keyset][key][1] if decksets.keys[keyset][key][1] != '' else "ico72_empty")
    label = decksets.keys[keyset][key][2]
